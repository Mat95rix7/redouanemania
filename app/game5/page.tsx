...
import { Sparkles, BookOpen, Clock, Trophy, Star } from 'lucide-react';
...
export default function Game5() {
  ...
  const handleReset = () => {
    // Optional: Add animation here too if you want the reset to fade out/in
    setIsAnimating(true);
    setTimeout(() => {
  ...
  const handleVerbSelect = (verb: string) => {
    ...
    // 2. Wait for fade out, then update state and fade back in
    setTimeout(() => {
        setShowStars(true);
        setSelectedVerb(verb);
        setIsAnimating(false);
    }, 300); // 300ms matches the duration in your CSS transition
  };
