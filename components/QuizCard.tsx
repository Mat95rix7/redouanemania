...
const QuizCard: React.FC<QuizCardProps> = ({ operation, correctAnswer, onAnswer, isActive }) => {
  // Utiliser operation comme clé d'état pour forcer la réinitialisation
  ...
  // Détecter le changement d'opération et réinitialiser l'état
  // Pattern React pour "adjusting state based on props"
  if (currentOp !== operation && isActive) {
    // Mise à jour pour déclencher le re-render avec état réinitialisé
    setCurrentOp(operation);
    setUserAnswer('');
    setStatus('idle');
    setTimeSpent(0);
    // Note: le timer sera nettoyé automatiquement par le useEffect
  }

  // Initialiser startTime au premier render ou quand l'opération change
  useEffect(() => {
    startTimeRef.current = Date.now();
  }, [currentOp, isActive]);

  // Démarrer le timer quand le composant devient actif
  useEffect(() => {
